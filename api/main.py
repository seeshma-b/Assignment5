...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    order = sandwiches.read_one(db, sandwich_id=sandwich_id)
    if order is None:
        raise HTTPException(status_code=404, detail="User not found")
    return order
...
def update_one_sandwich(sandwich_id: int, sandwich: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    order_db = sandwiches.read_one(db, sandwich_id=sandwich_id)
    if order_db is None:
        raise HTTPException(status_code=404, detail="User not found")
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    order = recipes.read_one(db, recipe_id=recipe_id)
    if order is None:
        raise HTTPException(status_code=404, detail="User not found")
    return order
...
def update_one_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    order_db = recipes.read_one(db, recipe_id=recipe_id)
    if order_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return recipes.update(db=db, recipe=recipe, recipe_id=recipe_id)


@app.delete("/recipes/{recipe_id}", tags=["Resources"])
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    order = resources.read_one(db, resource_id=resource_id)
    if order is None:
        raise HTTPException(status_code=404, detail="User not found")
    return order
...
def update_one_resource(resource_id: int, resource: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    order_db = resources.read_one(db, resource_id=resource_id)
    if order_db is None:
        raise HTTPException(status_code=404, detail="User not found")
